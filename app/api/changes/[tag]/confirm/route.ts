...
export async function POST(
  ...
  // confirming would commit into the detached mirror (and later
  // pushes would land in the author's branch). Refuse.
  if (task.remote === true) {
    return NextResponse.json(
      { error: "Задача опубликована другим пользователем — подтверждение недоступно" },
      { status: 403 },
    );
  }
...
async function handlePlanConfirm(
  req: NextRequest,
  config: { openspecDir: string; repos?: Record<string, RepoConfig> },
  ...
  for (const service of serviceNames) {
    ...
    const codeRepoPath = resolveRepoLocalPath(repoName, repoConfig);
    ...
    try {
      ...
      // Refresh origin/<base> before branching so the new
      // feature worktree is created off up-to-date remote
      // history, not stale local refs. We fail hard if the
      // remote or fetch is broken — better to surface that
      // here than to ship a feature branch off an old base.
      const baseBranch = repoConfig.branch;
      try {
        await fetchRemoteBranch(codeRepoPath, "origin", baseBranch);
      } catch (e) {
        throw new Error(
          `Не удалось обновить origin/${baseBranch}: ${(e as Error).message}`,
        );
      }
