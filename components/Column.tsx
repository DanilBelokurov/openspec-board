...
export function Column({ stage, meta, items, mode }: ColumnProps) {
  const Icon = meta.icon;
  return (
    <section className="flex h-full min-w-0 flex-1 flex-col rounded-lg bg-slate-100/60">
      <header className="flex items-center gap-2 px-3 py-2.5">
        <span className={`h-2 w-2 shrink-0 rounded-full ${STAGE_DOT[stage]}`} />
        <Icon className="h-3.5 w-3.5 shrink-0 text-slate-600" />
        <h2 className="truncate text-[12px] font-semibold uppercase tracking-wide text-slate-700">
          {meta.label}
        </h2>
        <span className="ml-auto shrink-0 rounded bg-slate-200/70 px-1.5 py-0.5 text-[10px] font-semibold text-slate-700">
          {items.length}
        </span>
      </header>
      ...
      <div className="flex flex-1 flex-col gap-2 overflow-y-auto px-2 pb-2 scrollbar-thin">
        {items.map((item) => (
          <SessionCard key={item.id} item={item} mode={mode} />
        ))}
        {items.length === 0 && (
