...
interface ReviewReadyCardProps {
  ...
  /**
   * Converted https URL of the remote, e.g. `https://github.com/user/repo`.
   * The card renders this as a clickable link to the branch.
   */
...
export function ReviewReadyCard({
  ...
  // Build the branch URL on GitHub / GitLab / etc. Falls
  // back to the remote URL root if the branch isn't known
  // yet (e.g. push succeeded but we somehow don't have the
  // branch field — defensive).
  const branchUrl =
    pushRemoteUrl && pushBranch
      ? `${pushRemoteUrl}/tree/${pushBranch}`
      : pushRemoteUrl ?? null;
